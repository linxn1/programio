...
public class PythonExecutionStrategy implements ExecutionStrategy {
    private static final long MAX_MEMORY_LIMIT = 5000;  // 设置最大内存限制为 50MB
    ...
    public CodeRespondTO executeCode(String language, String pythonScript, String userInput) {
        ...
        try {
            ...
            if (os.contains("win")) {
                // 创建执行 Python 脚本的进程
                processBuilder = new ProcessBuilder("python", "-c", pythonScript);
            } else {
                processBuilder = new ProcessBuilder("python3", "-c", pythonScript);
            }
